import { http, getApiData, postApiData, postFormData } from './client'
import type {
  ClientListItem,
  ImportCommitResult,
  ImportPreview,
...
/** GET /api/clients — paged client book for the current tenant (employees: assigned only). */
export function fetchClients(page = 1, pageSize = 20) {
  return getApiData<PagedResult<ClientListItem>>(`/api/clients?page=${page}&pageSize=${pageSize}&sortBy=companyName`)
}
